...
async function findPort() {
  ...
  throw new Error(`No free localhost port found from ${preferredPort} to ${preferredPort + portAttempts - 1}.`);
}

const port = await findPort();
if (port !== preferredPort) {
  console.warn(`Port ${preferredPort} is already in use; starting dev server on ${port}.`);
}
